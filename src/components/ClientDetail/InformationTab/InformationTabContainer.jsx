...
import * as clientDetailFunctions from './../ClientDetailContainer.jsx';
...
class InformationTabContainer extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      clientTypeOption: false, // did client option get clicked
  ...
  componentWillMount() {
    ...
    if (Object.keys(this.props.client).length > 0) {
      this.setState({
        ...
        holdingClientState: this.props.client,
        readOnlyOption: true,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!this.props.isConfirmed && nextProps.isConfirmed && this.state.deleteState) {
      ...
      this.props.editClientHandler({ ...profile, isActive: 0 });
    }
    ...
    switch (button) {
      case 'saveButton':
        ...
        if (emptyField === true) {
          ...
          delete profile.agencyName;
          if (this.state.putState) {
            this.props.editClientHandler({ ...profile, isActive: 1 });
          }
          if (this.state.createState) {
            this.props.addClientHandler({ ...profile, isActive: 1 });
          }
        }
        break;
      case 'editButton':
      ...
      case 'cancelButton':
        if (this.state.putState === true) {
          const propClient = this.props.client;
          this.setState({
            profile: propClient,
            readOnlyOption: true,
            saveCancelOption: false,
  ...
  render() {
    return (
      <InformationTabPresenter
        ...
        clientTypeOption={this.state.clientTypeOption}
      />
    );
  }
}
...
InformationTabContainer.propTypes = {
  ...
  fetchAgencies: React.PropTypes.func,
  fetchAccountManagers: React.PropTypes.func,
  fetchSpecialists: React.PropTypes.func,
  client: React.PropTypes.object,
  done: React.PropTypes.bool,
  isConfirmed: React.PropTypes.bool,
  editClientHandler: React.PropTypes.func,
  success: React.PropTypes.bool,
  showModal: React.PropTypes.func,
  updateClient: React.PropTypes.func,
  addClientHandler: React.PropTypes.func
};
...
const mapDispatchToProps = dispatch => ({
  ...
  addClientHandler: client => dispatch(clientActions.requestPostClient(client)),
  editClientHandler: client => dispatch(clientActions.requestPutClient(client)),
  deleteClientHandler: client => dispatch(clientActions.requestDeleteClient(client)),
